...
graph LR
    User[Campaign Brief] --> API[FastAPI /match]
    API --> Embed[all-MiniLM-L6-v2 Model]
    Embed --> DB[(ChromaDB Vector Store)]
    DB --> Rank[Cosine Distance Calculation]
    Rank --> Res[Ranked JSON Response]
...
### 2. Resolution of App Execution conflicts
